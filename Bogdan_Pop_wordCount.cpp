...
void readDataSet3()
{
	...
	char outputName[MAX_PATH];
	sprintf(fname, "D:\\Facultate\\An IV\\Semestrul I\\SRF\\Proiect\\datasets\\train.csv");
	sprintf(outputName, "D:\\Facultate\\An IV\\Semestrul I\\SRF\\Proiect\\datasets\\lengthTrainHam.csv");
	std::ifstream csv(fname);
...
void readDataSet4(Mat &X, Mat &Y, int features_num)
{
	...
	sprintf(outputName, "D:\\Facultate\\An IV\\Semestrul I\\SRF\\Proiect\\datasets\\wordCountTrainNoStopWords.csv");
	std::ifstream csv(fname);
	...
	X = Mat(total_num_train + 1, features_num, CV_32SC1);
...
{
	char fname[MAX_PATH];
	sprintf(fname, "D:\\Facultate\\An IV\\Semestrul I\\SRF\\Proiect\\datasets\\wordCountTrainNoStopWords.csv");
	std::ifstream csv(fname);
	
	//likelihood = Mat(nr_classes, features_num, CV_32FC1);
	int dimens[] = { nr_classes, features_num, 19 };
...
std::vector<int> read_one_test_line(int line, int features_num)
{
	char fname[MAX_PATH];
	char feat[MAX_PATH];
...
int classifyBayes(std::vector<int> test, Mat priors, Mat likelihood, int features_num)
{
...
float read_test(int features_num, Mat priors, Mat likelihood)
{
	...
	while (csv.good() && iterator <= total_num_test) {
		...
		int c = classifyBayes(words_count, priors, likelihood, features_num);
...
int main()
{
	...
	/*std::vector<int> words_count = read_one_test_line(2, features_num);
